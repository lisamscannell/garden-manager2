...
app.get("/api/seeds", (req, res) => {
  const rows = db
    .prepare("SELECT * FROM seeds ORDER BY plantType, variety")
    .all();
...
app.post("/api/seeds", (req, res) => {
  ...
  res.json(
    fromRow(db.prepare("SELECT * FROM seeds WHERE id = ?").get(seed.id)),
  );
});
...
function generateEventId(seedId, actualSowDate, plannedSowDate) {
  const d =
    actualSowDate || plannedSowDate || new Date().toISOString().split("T")[0];
  const datePart = d.slice(2, 4) + d.slice(5, 7) + d.slice(8, 10);
  const seedRowId =
    db.prepare("SELECT rowid FROM seeds WHERE id = ?").get(seedId)?.rowid ??
    seedId;
...
app.get("/api/tasks/sowing/:sowingEventId", (req, res) => {
  const rows = db
    .prepare("SELECT * FROM tasks WHERE sowingEventId = ? ORDER BY dueDate ASC")
...
// Nginx serves the frontend in production — Express only handles /api routes
